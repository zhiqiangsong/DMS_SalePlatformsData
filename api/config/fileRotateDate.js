...
function getDate() {
	var d = new Date();
	return (d.getYear()+1900).toString() + pad2(d.getMonth() + 1) + pad2(d.getDate()) + "_" + pad2(d.getHours()) + pad2(d.getMinutes()) + pad2(d.getSeconds())+"_"+d.getMilliseconds();
}
...
FileRotateDate.prototype.rotateFile = function () {
	...
	var dateStr = getDate();
	var targetFN = path.join(this.dirname, basename + dateStr +".bak");

/*	var startingOpt = 96;

	while (1) {
		// try the normal filename first (no a-z added)
		for (var opt = startingOpt; opt < 122; ++opt) {
			var targetFN = path.join(this.dirname, basename + dateStr + (opt === 96 ? '' : String.fromCharCode(opt) ) + ".bak");
